// // import React from 'react'
// // import { Box, Stack } from "@chakra-ui/react"
// // import Card from './Card'
// // import axios from "axios";


// // const Home = () => {


// //     const checkoutHandler = async (amount) => {

// //         const { data: { key } } = await axios.get("http://www.localhost:3000/api/getkey")

// //         const { data: { order } } = await axios.post("http://localhost:3000/api/checkout", {
// //             amount
// //         })

// //         const options = {
// //             key,
// //             amount: order.amount,
// //             currency: "INR",
// //             name: "Snehal",
// //             description: "Sweets and Snacks",
// //             image: "https://avatars.githubusercontent.com/u/25058652?v=4",
// //             order_id: order.id,
// //             callback_url: "http://localhost:3000/api/paymentverification",
// //             prefill: {
// //                 name: "Snehal Mhasavekar",
// //                 email: "snehalmhasavekar27@gmail.com",
// //                 contact: "9307885370"
// //             },
// //             notes: {
// //                 "address": "Gargoti"
// //             },
// //             theme: {
// //                 "color": "#121212"
// //             }
// //         };
// //         const razor = new window.Razorpay(options);
// //         razor.open();
// //     }

// //     return (
// //         <Box>

// //             <Stack h={"100vh"} alignItems="center" justifyContent="center" direction={["column", "row"]}>

// //                 <Card amount={5000} img={"https://cdn.shopify.com/s/files/1/1684/4603/products/MacBookPro13_Mid2012_NonRetina_Silver.png"} checkoutHandler={checkoutHandler} />
// //                 <Card amount={3000} img={"http://i1.adis.ws/i/canon/eos-r5_front_rf24-105mmf4lisusm_32c26ad194234d42b3cd9e582a21c99b"} checkoutHandler={checkoutHandler} />

// //             </Stack>
// //         </Box>
// //     )
// // }

// // export default Home
// import React from 'react';
// import { Box, Stack } from "@chakra-ui/react";
// import Card from './Card';
// import axios from "axios";

// const Home = () => {

//     const checkoutHandler = async (amount) => {
//         try {
//             // Make a GET request for the key
//             const { data: { key } } = await axios.get("http://localhost:3000/api/getkey");

//             // Make a POST request for checkout
//             const { data: { order } } = await axios.post("http://localhost:3000/api/checkout", {
//                 amount
//             });

//             // Construct Razorpay options
//             const options = {
//                 key,
//                 amount: order.amount,
//                 currency: "INR",
//                 name: "Snehal Mhasavekar",
//                 description: "Sweets and Snacks",
//                 image: "https://avatars.githubusercontent.com/u/25058652?v=4",
//                 order_id: order.id,
//                 callback_url: "http://localhost:3000/api/paymentverification",
//                 prefill: {
//                     name: "Snehal Mhasavekar",
//                     email: "snehalmhasavekar27@gmail.com",
//                     contact: "9307885370"
//                 },
//                 notes: {
//                     "address": "Gargoti"
//                 },
//                 theme: {
//                     "color": "#121212"
//                 }
//             };
            
//             // Create and open the Razorpay instance
//             const razor = new window.Razorpay(options);
//             razor.open();
//         } catch (error) {
//             console.error("Error during payment:", error);
//             // You can handle the error in your application, e.g., show an error message to the user.
//         }
//     }

//     return (
//         <Box>
//             <Stack h={"100vh"} alignItems="center" justifyContent="center" direction={["column", "row"]}>
//                 <Card amount={5000} img={"https://cdn.shopify.com/s/files/1/1684/4603/products/MacBookPro13_Mid2012_NonRetina_Silver.png"} checkoutHandler={checkoutHandler} />
//                 <Card amount={3000} img={"http://i1.adis.ws/i/canon/eos-r5_front_rf24-105mmf4lisusm_32c26ad194234d42b3cd9e582a21c99b"} checkoutHandler={checkoutHandler} />
//             </Stack>
//         </Box>
//     );
// }

// export default Home;

import React from 'react';
import { Box, Stack } from "@chakra-ui/react";
import Card from './Card';
import axios from "axios";

const Home = () => {

    const checkoutHandler = async (amount) => {
        try {
            // Make a GET request for the key
            const { data: { key } } = await axios.get("http://localhost:3000/api/getkey");

            // Make a POST request for checkout
            const { data: { order } } = await axios.post("http://localhost:3000/api/checkout", {
                amount
            });

            // Construct Razorpay options
            const options = {
                key,
                amount: order.amount,
                currency: "INR",
                name: "Snehal",
                description: "Sweets and Snacks",
                image: "https://avatars.githubusercontent.com/u/25058652?v=4",
                order_id: order.id,
                callback_url: "http://localhost:3000/api/paymentverification",
                prefill: {
                    name: "Snehal",
                    email: "snehalmhasavekar27@gmail.com",
                    contact: "9307885370"
                },
                notes: {
                    "address": "Gargoti"
                },
                theme: {
                    "color": "#121212"
                }
            };

            // Create and open the Razorpay instance
            const razor = new window.Razorpay(options);
            razor.open();
        } catch (error) {
            console.error("Error during payment:", error);
            // You can handle the error in your application, e.g., show an error message to the user.
        }
    }

    return (
        <Box>
            <Stack h={"100vh"} alignItems="center" justifyContent="center" direction={["column", "row"]}>
                <Card amount={5000} img={"https://cdn.shopify.com/s/files/1/1684/4603/products/MacBookPro13_Mid2012_NonRetina_Silver.png"} checkoutHandler={checkoutHandler} />
                <Card amount={3000} img={"http://i1.adis.ws/i/canon/eos-r5_front_rf24-105mmf4lisusm_32c26ad194234d42b3cd9e582a21c99b"} checkoutHandler={checkoutHandler} />
            </Stack>
        </Box>
    );
}

export default Home;

